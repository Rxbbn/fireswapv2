...
import phx from '../token-details/phx.json'
import phxtestnet from '../token-details/phx-testnet.json'
    ...
    useMemo(()=>{
        if(chainId === 1){
           ...
           setinputdetails(inputtoken)
            ...
            const inputtoken = phx.filter(token => input ? swapDetails.INPUT.currencyId === token.symbol : swapDetails.INPUT.currencyId === token.address)
            const outputtoken = phx.filter(token => output ? swapDetails.OUTPUT.currencyId === token.symbol : swapDetails.OUTPUT.currencyId === token.address)
            setinputdetails(inputtoken)
            setoutputdetails(outputtoken)
        }else if(chainId === 13382){
            const inputtoken = phxtestnet.filter(token => input ? swapDetails.INPUT.currencyId === token.symbol : swapDetails.INPUT.currencyId === token.address)
            const outputtoken = phxtestnet.filter(token => output ? swapDetails.OUTPUT.currencyId === token.symbol : swapDetails.OUTPUT.currencyId === token.address)
            setinputdetails(inputtoken)
            setoutputdetails(outputtoken)
        }
    },[swapDetails,chainId])
